...
namespace bezier {
	...
	namespace details {
		...
		struct Bernstein<2, n> {
			...
			{
				switch (n) {
					case 0: return pow2(T(1) - x);
					case 1: return T(2)*x*(T(1) - x);
		...
		struct Bernstein<3, n> {
			...
			{
				switch (n) {
					case 0: return pow3(T(1) - x);
					case 1: return T(3)*x*pow2(T(1) - x);
					case 2: return T(3)*pow2(x)*(T(1) - x);
		...
		struct Bernstein<4, n> {
			...
			static T value(T x)
			{
				switch (n) {
					...
					case 1: return T(4)*x*pow3(T(1) - x);
					case 2: return T(6)*pow2(x)*pow2(T(1) - x);
					case 3: return T(4)*pow3(x)*(T(1) - x);
	...
	{ 
		return details::sample(&points[0], t);
	}

	template <typename Point, typename T, size_t size>
	Point sample_weight(const std::array<Point, size>& points, const std::array<T, size>& weights, T t)
	{
		return details::sample_weight<size - 1>(&points[0], &weights[0], t);
	}

	using details::sample_patch;
